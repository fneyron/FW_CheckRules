...
mysql -u root -D fw_logs -e "Call createRangeIP();"
mysql -u root -D fw_logs < checkRules.sql
mysql -u root -D fw_logs -e "Call checkRules();"
mysql -u root -D fw_logs -e "SELECT Source, Destination, Service INTO OUTFILE '$drop' FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' FROM logs l, dropped d WHERE d.Number=l.Number;"
